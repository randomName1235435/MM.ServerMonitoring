...
public class InsertCommandTest
{
    ...
    private Mock<IInsertValidator<MonitorActionExecution>> validatioMock;

    [TestInitialize]
    public void Initialize()
    {
        this.validatioMock = new Mock<IInsertValidator<MonitorActionExecution>>();
        this.crudServiceMock = new Mock<ICrudServiceAsync<MonitorActionExecution>>();
        this.param = new MonitorActionExecution();
    }

    private InsertCommand<MonitorActionExecution, ICrudServiceAsync<MonitorActionExecution>> CreateInstance()
    {
        return new InsertCommand<MonitorActionExecution, ICrudServiceAsync<MonitorActionExecution>>(
    ...
    {
        var instance = new InsertCommand<MonitorActionExecution, ICrudServiceAsync<MonitorActionExecution>>(null, null);
    }
    ...
    public async Task ExecuteAsync_ValidParam_ShouldSucceed()
    {
        ...
        this.crudServiceMock
            .Setup(m => m.InsertAsync(It.IsAny<MonitorActionExecution>(), It.IsAny<CancellationToken>()))
    ...
    public void Execute_ValidParam_ShouldSucceed()
    {
        ...
        var called = false;
        this.SetupValidationSuccess();
        this.crudServiceMock.Setup(m => m.Insert(It.IsAny<MonitorActionExecution>())).Callback(() => called = true);
        this.CreateAndExecute(this.param);
        Assert.IsTrue(called);
    }
